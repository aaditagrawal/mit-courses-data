        ...
        (sem.core_courses || []).forEach((code: string) => {
          ...
          if (normalized.includes("****")) {
            results.placeholder_codes.push({
              ...
              file: degreeFile,
              code,
              context: `Core course in Sem ${sem.sem_index}`,
            });
          } else if (
            !courseMap.has(normalized) &&
            !normalized.endsWith("4191") &&
            !normalized.endsWith("4292") &&
            !normalized.endsWith("4293")
          ) {
            results.missing_courses.push({
              degree: degreeName,
              file: degreeFile,
              code,
              context: `Core course in Sem ${sem.sem_index}`,
        ...
        (sem.elective_slots || []).forEach((slot: any) => {
          if (slot.course_code) {
            ...
            if (normalized.includes("****")) {
              results.placeholder_codes.push({
                ...
                context: `Elective slot ${slot.label} in Sem ${sem.sem_index}`,
              });
            } else if (
              !courseMap.has(normalized) &&
              !normalized.endsWith("4191") &&
          ...
          if (slot.pool_ref) {
            if (!data.structure.elective_pools[slot.pool_ref]) {
              results.missing_pools.push({
                degree: degreeName,
                file: degreeFile,
                pool: slot.pool_ref,
                context: `Slot ${slot.label} in Sem ${sem.sem_index}`,
              });
            }
          }
            ...
            slot.courses.forEach((code: string) => {
              ...
              if (normalized.includes("****")) {
                results.placeholder_codes.push({
                  ...
                  file: degreeFile,
                  code,
                  context: `Inline elective in slot ${slot.label} Sem ${sem.sem_index}`,
                });
              } else if (
                !courseMap.has(normalized) &&
                !normalized.endsWith("4191") &&
