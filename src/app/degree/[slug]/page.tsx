      ...
      <header className="sticky top-0 z-50 w-full border-b bg-background/80 backdrop-blur-md border-border">
        <div className="container flex h-14 items-center pl-6 max-w-[1400px] mx-auto">
          ...
          <div className="flex flex-1 items-center justify-between overflow-hidden">
            <h1 className="text-sm font-semibold tracking-tight truncate mr-4 text-foreground uppercase tracking-wider">
              {degree_metadata.title}
            </h1>
            ...
            <table className="w-full text-sm text-left min-w-[600px] sm:min-w-full">
              <thead className="text-xs text-muted-foreground uppercase bg-muted/30 border-b border-border">
                <tr>
                  ...
                  <th className="px-4 py-2 font-medium w-16 text-right border-l border-border">
                    C
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border bg-background">
                ...
                {semester.core_courses.length === 0 && (
                  <tr>
                    <td
                      colSpan={6}
                      className="px-4 py-8 text-center text-muted-foreground italic text-xs"
                    >
  ...
  if (isMissing && (is4191 || is4293 || is4292)) {
    ...
    return (
      <tr className="group hover:bg-muted/30 transition-colors">
        ...
        <td className="px-4 py-3 text-right hidden sm:table-cell text-muted-foreground font-mono text-xs">
          -
        </td>
        <td className="px-4 py-3 text-right hidden sm:table-cell text-muted-foreground font-mono text-xs">
          -
        ...
        <td className="px-4 py-3 text-right border-l border-border font-mono font-bold text-foreground">
          {credits}
        </td>
      </tr>
    );
  ...
  if (isMissing) {
    ...
    return (
      <tr className="group hover:bg-muted/30 transition-colors">
        ...
        <td className="px-4 py-3 text-right hidden sm:table-cell text-muted-foreground/30 font-mono text-xs">
          -
        </td>
        <td className="px-4 py-3 text-right hidden sm:table-cell text-muted-foreground/30 font-mono text-xs">
          -
        ...
        <td className="px-4 py-3 text-right border-l border-border font-mono text-muted-foreground/50">
          -
        </td>
      </tr>
    );
        ...
        <div className="p-2 bg-background">
          ...
          {displayMode === "simple" &&
            simpleCourses.map((c, idx) => (
              <div
                key={`${c}-${idx}`}
                className="flex items-center justify-between p-2 rounded hover:bg-muted/30 group/item transition-colors"
              >
                <div className="flex items-center gap-3">
                  <span className="font-mono text-xs font-semibold text-muted-foreground w-20">
                    {c}
                  </span>
                  <Link
