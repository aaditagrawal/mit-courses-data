 ...
 */
function directScore(corpus: CourseCorpus, ordinal: number, matched: number, query: string, compactQuery: string): number {
...
function scoreCourses(query: string): ScoredCandidates {
    ...
    const sweep = superset
        ...
        : (field: FieldBuffer, needle: string, bit: number) =>
            forEachMatch(field, needle, mark(bit));
 ...
 */
function selectTop(slots: Int32Array, count: number, need: number, scores: Float64Array, ordinals: Int32Array): void {
    let lo = 0;
    let hi = count - 1;
    ...
    while (lo < hi) {
        ...
        const mid = (lo + hi) >> 1;
        if (compareRank(slots[mid], slots[lo], scores, ordinals) < 0) [slots[lo], slots[mid]] = [slots[mid], slots[lo]];
        if (compareRank(slots[hi], slots[lo], scores, ordinals) < 0) [slots[lo], slots[hi]] = [slots[hi], slots[lo]];
        if (compareRank(slots[hi], slots[mid], scores, ordinals) < 0) [slots[mid], slots[hi]] = [slots[hi], slots[mid]];
...
function selectCourses(options: CourseSearchOptions, limit: number, offset: number): { data: SearchResult[]; total: number } {
    const corpus = getCourseCorpus();
...
function selectDegrees(query: string, limit: number, offset: number): { data: DegreeSummary[]; total: number } {
...
let departmentSummariesCache: Array<{ slug: string; name: string; courseCount: number }> | null = null;
